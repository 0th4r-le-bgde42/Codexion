...
/*   Updated: 2026/03/04 14:56:20 by ldauber          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int init_dongles(t_data *data)
{
	...
	{
		int mumu = pthread_mutex_init(&data->dongles[i].mutex, NULL);
		if (mumu != 0)
			return (printf("free_all"));
		pthread_cond_init(&data->dongles[i].cond, NULL);
		data->dongles[i].last_release_time = 0;

		data->dongles[i].request_queue.array = malloc(sizeof(t_coder *) * data->config.num_coders);
...
int init_coders(t_data *data)
{
	...
	while (i < data->config.num_coders)
	{
		...
		data->coder[i].right_dongle = &data->dongles[(i + 1) % data->config.num_coders];
		i++;
	}
	return (0);
}